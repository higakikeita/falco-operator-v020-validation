...
2026 年は Falco が OSS として公開されてから **10 周年**（2016 → 2018 CNCF Sandbox → 2020 Incubating → 2024 Graduated）。その節目に KubeCon EU 2026 (2026-03-24) で maintainers の Aldo Lacuku (Kong) と Iacopo Rozzo (Sysdig) が発表したのが **"In Falco's Nest: The Evolution of Cloud Native Runtime Security"**。そこで「最初の production-ready 版」として紹介された **Falco Operator v0.2.0**（2026-03-23 release）を、実際に EKS で 10 シナリオ・3 日間ほど触り倒したのが本記事です。

結論を先に書くと、**17 個の非自明なハマりどころ**が見つかりました。本文は KubeCon talk の公式コンテクストと突き合わせながら、3 章に整理した運用観点のレビューです。
...
### 1.4 What's New in Falco 0.42 / 0.43（KubeCon talk より）

Operator の話に入る前に、足元の Falco engine 側の進化も触れておきます。KubeCon EU 2026 で発表された 0.42 / 0.43 の主要改善：

| 機能 | バージョン | 効果 |
|---|---|---|
| **Drop enter initiative** | 0.42 | カーネル instrumentation を軽量化 — **latency -20%、Falco CPU 使用量 -30%**。enter event を取らなくなったため `evt.dir` が **deprecated**。Issue: [libs#2588](https://github.com/falcosecurity/libs/issues/2588) |
| **Capture recording / Stratoshark** | 0.42 | Rule に `capture: true` と `capture_duration: 5000` を書けるようになり、検知時のシステム挙動を [Stratoshark](https://stratoshark.org/) でフォレンジック解析可能。Issue: [falco#3595](https://github.com/falcosecurity/falco/issues/3595) |
| Plugin event schema versioning | 0.42 | Plugin の互換性管理 |
| Thread table auto-purging | 0.42 | メモリ圧迫の自動緩和 |
| Container plugin perf/stability | 0.43 | エコシステム最重要 plugin の安定化 |
| gRPC output / gVisor engine deprecated | 0.43 | サポート集約 |

特に **「Drop enter initiative」**は本記事の検証中にも `falcoctl -V` の warning として出てきており、自作ルールから `evt.dir = <` を抜く必要がありました（Chapter 2 で再登場）。

### 1.5 native sidecar (K8s 1.29+)
...
なお KubeCon の architecture overview スライドには **「SSA (Server-Side Apply) for conflict-free reconciliation」が公式要件として明記**されています。つまり Operator は **SSA 前提で reconcile ロジックを書いている**ため、これから登場する Finding #1（`kubectl apply` が 262KB 制限で死ぬ）は実装ミスではなく仕様。`--server-side` を使うのが正しい手順、というのが maintainers 側のスタンスです。
...
**サイドノート：OCI 以外の選択肢もある**

v0.2.0 から **`configMapRef` で Rulesfile / Config を ConfigMap から読む**こともできます。
...
OCI artifact のような immutable version 管理は得られませんが、**社内 Slack / Notion から rule を貼り付けて検証** みたいなライトな使い方には十分。本記事では「OCI に振り切った場合の現実」を見るために OCI 一辺倒で検証しましたが、選択肢として頭の隅に入れておくと吉。
...
Falcosidekick の売りは KubeCon talk でも強調された **70+ integrations**（Slack / Teams / S3 / Kafka / Elasticsearch / Loki / PagerDuty / ...）。`WEBHOOK_ADDRESS` 環境変数を Component CRD で注入すれば webhook.site などへの forward は即動きます。が、**`SLACK_WEBHOOKURL` を取得しに行くと**、enterprise Slack workspace（Sysdig 含む）では `Incoming Webhooks` の app install に **admin 承認が必要**で、開発者が手元で完結できません。
...
| #9 | artifact-operator が `secret-in-use` finalizer を貼る → delete が hang | `kubectl patch ... -p '{"metadata":{"finalizers":[]}}'`。**KubeCon arch 図に "ConfigMaps / Secrets: Protected by finalizers" と明示**されており仕様 |
| #10 | ECR token は **12h で expire**、in-cluster Secret も腐る | CronJob で rotate、IRSA + ECR helper |
| #11 | `spawned_process` / `container` macro が default OCI rule に**含まれていない** | inline で `evt.type=execve and container.id != host` を書く。なお `evt.dir = <` も書くと **Drop enter initiative (0.42)** の影響で **deprecated 警告**が出るため不要 |
...
### 3.5 What's Next — Falco 0.44 以降の見どころ（KubeCon talk より）

検証中の現実は上記の通りですが、KubeCon EU 2026 で発表された **Falco 0.44 以降の roadmap** を見ると、本記事で挙げた多くの摩擦は将来解消される見込みです。

| 機能 | ステータス | 何が解決するか |
|---|---|---|
| **BPF Iterators**（0.44） | 開発中 | procfs スキャンより高速な thread 情報取得。state recovery 高速化。[libs#2879](https://github.com/falcosecurity/libs/issues/2879) |
| **Multi-thread support** | Soon in Tech Preview | 現在の single-threaded event loop は high-core machine（数百コア時代）で saturate しやすい。per-CPU ring buffer → worker thread fan-out で水平拡張。[falco#3749](https://github.com/falcosecurity/falco/issues/3749) |
| **Falco LSP** | Soon on VSCode Marketplace | **VS Code 拡張で rule 編集のリアルタイム診断**（syntax errors、undefined macro、unknown fields、source mismatches）、code completion、hover doc、go-to-definition、CI validation。**Finding #11（macro 未定義に気付けない問題）の決定打**になる。[falco-lsp v0.1.1 (Incubating)](https://github.com/falcosecurity/falco-lsp) |
| **Coding Agents Kit** | Tech Preview | AI coding agent が Falco rule を書くための tooling。[leogr/coding-agents-kit](https://github.com/leogr/coding-agents-kit) |

特に **Falco LSP は Runtime Security as Code を本気で言うなら必須のピース**で、これが安定すれば「rule を YAML で書く心理的ハードル」が下がり、検知ロジックの民主化が一段進みます。今回の検証で詰まった macro 未定義や `evt.dir` deprecation などは **CI で `falco-lang validate` を回すだけで防げる**ようになる。

加えて、Falco 10 周年を記念して **Sysdig が $70,000 を CNCF に寄付**（feature development grant / contributor stipend / technical writer stipend に使われる）— OSS としての継続投資コミットメントも合わせて見ておきたい話。

### 3.6 結論：v0.2.0 は **半分** Runtime Security as Code
...
v0.3 で Finding #13（tag 変更で再 pull されない）が解消され、Falco LSP が GA すれば、**Runtime Security as Code は本当に "as Code" になる**と思っています。Falco 10 周年の節目に v0.2.0 が来たのは象徴的で、次の 10 年は「rule を image と同じ思想で管理する」が当たり前になるはず。
